...
public class MotorTableModelUsinada extends AbstractTableModel {
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		case 1:
			if(dados.getPontosDeLeituras().size()!=0){
				
				 return new controlaUsinada().formataApenasData(dados.getPontosDeLeituras().get(0).getData());
			}else{
				return null;
			}
		...
		case 2:
			...
			if(dados.getPontosDeLeituras().size()!=0){
				 ...
				 return new controlaUsinada().formataHoraMinutosSegundos(dados.getPontosDeLeituras().get(0).getData());
			}else{
				return null;
			}
		case 3:
